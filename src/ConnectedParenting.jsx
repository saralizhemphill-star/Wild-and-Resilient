...
function TriangleSections({ sections, openSections, setOpenSections }) {
  // We use the first 3 sections for the triangles
    ...
    <div className="space-y-0">
      ...
      <div className="relative flex justify-center items-end gap-0 mb-6" style={{ minHeight: "280px" }}>
        {triSections.map((section, si) => {
          ...
          const c = colors[si];
          // Position: left, center (taller), right
          ...
          return (
            ...
            >
              {/* Triangle shape via CSS clip-path */}
              <div
...
export default function ConnectedParenting() {
  ...
  // ===== PDF Worksheet filenames by lesson ID =====
  ...
  if (page === "lesson" && activeModule === 1 && activeLesson !== null) {
    ...
    // Section 4 (index 3) is the "Practical Strategies" section
  ...
  ></iframe>
          ...
          {/* TAB: Lesson Content — TRIANGLE LAYOUT for sections 1-3 */}
          {lessonTab === "content" && (
            <div className="space-y-4">
          ...
          {/* TAB: Practical Strategies (replaces Worksheet) */}
          {lessonTab === "strategies" && (
            <div className="space-y-4">
              {/* Practical Strategies Content (from section 4 of lesson content) */}
              <div className="bg-white rounded-xl border border-gray-200 p-6">
                ...
                <div className="space-y-4">
                  {practicalSection.content.split("\n\n").map((para, pi) => {
                    // Check if this paragraph starts with a number (numbered strategy)
                    const numberMatch = para.match(/^(\d+)\.\s+(.+)/);
                    if (numberMatch) {
                      const [, num, text] = numberMatch;
                      // Split on the first period to get the bold title
